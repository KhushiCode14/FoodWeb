...
const Navbar: React.FC = () => {
  ...
  return (
    <div className=" mt-2 flex justify-between text-white">
      ...
      <div className={`flex flex-col md:flex-row md:items-center md:justify-between w-full md:w-auto max-w-screen-xl ${menu ? 'block' : 'hidden md:block'}`} >
      <ul className="flex flex-col md:flex-row md:space-y-4"> 
        {
          navLink.map((link, index) => (
            <Link key={index} to={link.to} className="mx-3 hover:text-gray-800">
              {link.linkName}
